...
function IconGrid(name, hostElement, datasource, layout) {
  ...
  this.dashboard = undefined; //created on initialize
  ...
  //caches the constructed grid item panes for speed, and to stop poking the
  ...
  //var _dashOffsetY;
  //currently unused, since we don't support vertically scrolling dashboard
  // at the moment
  this._appIcon;
  this._pageAnimating = false;

  //application dragging/rearranging globals
  //timer that determines whether we are dragging the app or the dashboard  
  this._mouseDownHoldTimer;
  //timer that retains the mouse for a brief time after they user drags out of the window
  this._mouseDragoutTimer;
  //timer that retains the mouse for a brief time after they user drags out of the window
  this._pageScrollTimer;
  //true if scrolling to new page should wait, false if it can go ahead
  this._pageScrollDelay = false;
  // this is a cached event object, used for triggering multi-page scrolls when the user doesn't
  ...
  // the previous slot the currently dragged app was over, used to trigger animations
  // only when the app is moved over a different slot
  this._draggedAppLastSlot;
};

IconGrid.prototype = {
  getPage: function () {
    var left = Math.abs(this.dashboard.position().left);
    var panelWidth = this.layout.panelWidth;
    return Math.floor((left + panelWidth / 2) / panelWidth);
  },

  goToPage: function (page, animationSpeed, completionCallback) {
    var pagesCount = this.dashboardState.pages.length;
    if (page >= pagesCount)
      page = pagesCount - 1;
    else if (page < 0)
      page = 0;
    ...
    var panelWidth = this.layout.panelWidth;
    var targetLeft = (page * panelWidth * -1);
    if ((this.dashboard.position().left === targetLeft) || this._pageAnimating) {
      return page;
    }

    this._pageAnimating = true;
    // used by the auto-scrolling code to prevent rapid scrolling across many pages
    this._pageScrollDelay = true;

    var self = this;
    this.dashboard.animate({ left: targetLeft }, animationSpeed, function () {
      self._pageAnimating = false;

      if (completionCallback) {
        completionCallback(page, self);
      }
  ...
  _arrangeAppsOnPageToFit: function (pageIdx, overSlot) {
    ...
    if (arrangedPage[overSlot]) {
       ...
       * prefer a hole that is to the left of the dragged item, and as close as possible to it,
       * so as to move only the apps that must be moved, and no others.
       * If we are unable to find a hole to the left, then find the closest one to the right of
       * the dragged item. If there are no holes to be found, then use a virtual hole that is off
       * the end of the array to the right. We will fix the array up after the drop.
       */

      //try to find a left hole
      var hole = (this.layout.rowCount * this.layout.columnCount);
      for (var i = 0; i < overSlot; i ++) {
        if (!arrangedPage[i]) {
          hole = i;
        }
      }
    ...
    if (arrangedPage) {
      ...
      // NOTE: (this.layout.columnCount+1) here is important!  It pushes the rightmost one off the side of the
      // page, so that it is hidden
      for (var i = 0; i < (this.layout.rowCount * this.layout.columnCount) + 1; i ++) {
        if (arrangedPage[i]) {
          var pos = this._positionForSlot(i);
  ...
  _redrawPage: function (page, animated) {
  ...
  _fixUpPageOverflows: function (startPage) {
    /* Loop over all the pages, starting at the first, (or maybe the one we just dropped on?) and
     * check to make sure none of the apps have run off the end of the page.  If so, then shove
     * them onto the next page, and keep going, until we get to the end, or a page that doesn't need to be fixed.
    ...
    for (var p = startPage; p < numPages; p ++) {
      if (this.dashboardState.pages[p][pageSize]) { //overflow
        ...
        this._redrawPage(p + 1);
      }
    }

    var emptyish;
    //remove empty trailing pages.  this must be the last code in this function, since we return
  ...
  _slotForPosition: function (left, top) {
  ...
  _onMouseDown: function (e) {
    ...
    this._dashboardScrollOffsetX = parseInt(this.dashboard.position().left, 10) || 0;
  ...
  _onMouseMove: function (e) {
    ...
    // give the user some forgiveness when pressing and holding. if they only move a couple of pixels,
    // we will stillcount it as a hold.
    ...
    // this is the -app- dragging code, which manages the necessary animations, the underlying data
    // changes, and the possible paging to a different dashboard page while carrying an app
    var curPage = this.getPage();
    if (this._draggedApp) {
      // we are moving the appDisplayFrame from one coordinate system to another, so we need to computer an
      // offset, so it doesn't jump away from the cursor
      ...
      //frame padding
      var dragOutAmount = Math.floor((this.layout.itemBoxWidth - theDraggedItem.children(".icon").width() ) / 2) - 1;
      ...
      var newLeft = Math.min(Math.max((this._draggedAppOffsetX + e.clientX - this._mouseDownX),
        -dragOutAmount), (this.layout.panelWidth + dragOutAmount - this.layout.itemBoxWidth));
      var newTop = Math.min(Math.max((this._draggedAppOffsetY + e.clientY - this._mouseDownY), 0),
        (this.layout.panelHeight - this.layout.itemBoxHeight));
      ...
      var currentSlot = this._slotForPosition(theDraggedItem.position().left, theDraggedItem.position().top);
      ...
      if (paging !== 0) {
        ...
        if (!this._pageScrollDelay) {
          var resultantPage = this.goToPage(curPage + paging, 400, function (page, self) {
            ...
            if (curPage !== page)
              self._redrawPage(curPage);
            self._arrangeAppsOnPageToFit(page, currentSlot);
            self.updatePageIndicator();
          });
        }
      }
      // otherwise, if they are over a new slot than they were last time, we animate the icons on the page into
      // the proper configuration      
        ...
        this._arrangeAppsOnPageToFit(curPage, this._draggedAppLastSlot);
  ...
  // let's actually lift the app up and out of the page, and attach it to the container, so
  ...
  _onMouseHold: function (e) {
    ...
    if (this._draggedApp) {
      ...
      this._draggedAppOffsetX = parseInt(this.gridItemCache[this._draggedApp].position().left, 10) || 0;
      this._draggedAppOffsetY = parseInt(this.gridItemCache[this._draggedApp].position().top, 10) || 0;

      var startSlot = this._slotForPosition(this._draggedAppOffsetX, this._draggedAppOffsetY)

      //remove the app from the page it started on
      this.dashboardState.pages[this.getPage()][startSlot] = undefined;
      //lift it up
      this._draggedAppOrigZ = this.gridItemCache[this._draggedApp].css('z-index');

      //remove it from the page it was in and attach it to the container instead
      this.gridItemCache[this._draggedApp].css('z-index', 10000);
      this.gridItemCache[this._draggedApp].detach();
      this.container.append(this.gridItemCache[this._draggedApp]);
  ...
  _onMouseUp: function (e) {
    ...
    if (this._draggedApp) {
      ...
      //    - if page was full before dropping, then all apps afterwards need to be shifted over, possibly
      // changing every page afterward 
      ...
      var currentSlot = this._slotForPosition(this.gridItemCache[this._draggedApp].position().left,
        this.gridItemCache[this._draggedApp].position().top);

      var rearrangedApps = this._arrangeAppsOnPageToFit(curPage, currentSlot);
      ...
      // DO LOTS OF FIXUP!!
      this._fixUpPageOverflows(curPage);

      // save the changes
      this.saveIconGridState(this.name, this.dashboardState);

      // remove the appDisplayFrame from the container
      this.gridItemCache[this._draggedApp].detach();
      ...
      var pos = this._positionForSlot(currentSlot);
   ...
   * There are three moving parts to worry about:
   *  1. Dataset (list of currently installed apps)
   *  2. DashboardState (a two-dimensional arrangement of references to 
   *     items in the Dataset),
   *  3. DisplayCache (a keyed object containing visual representations
   *     of every item in the DashboardState)
   *
   * 1) Poll or get notified that the dataset may have changed
   *
   * 2) For every item in the DashboardState, if it does not appear in the
   *    Dataset, remove it from the DashboardState.  
   *    Also remove the corresponding item from the DisplayCache, and from
   *    the Page it was displayed on.
   *
   *  3) For every item in the Dataset, if it does not appear in the
   *     DashboardState, then add it into the DashboardState, in the first
   *     empty slot.
   *     Also generate the DisplayCache item, store it, and add it to the
   *     corresponding Page.
   *
  ...
  // handle requests for the dashboard state
  ...
  refresh: function () {
    ...
    var newState = this.loadIconGridState(this.name);

    // this is the function called by both the navigator.apps callback or the jetpack
    // callback when the dashboard state loads we immediately turn around and load the app list.
    if (newState)
    ...
    for (p = 0; p < this.dashboardState.pages.length; p ++) {
      ...
      for (s = 0; s < this.layout.rowCount * this.layout.columnCount; s ++) {
        ...
        if (guid && !this.gridItemCache[guid]) {
          ...
          var pos = this._positionForSlot(s);
  ...
  updatePageIndicator: function () {
    ...
    for (var p = 0; p < this.dashboardState.pages.length; p ++) {
      ...
      var indi = $("<img width='" + Math.floor(this.layout.pageindicatorHeight * 0.8) + "' height='" +
        Math.floor(this.layout.pageindicatorHeight * 0.8) + "'/>");
      indi.attr('src', (p === currentPage) ? cursor_path + "page_cur.png" : cursor_path + "page.png");
  ...
  createGridItem: function (guid) {
    ...
    var darken = $("<div/>").addClass("iconshader").css({
  ...
  initialize: function () {
    ...
    this.container.css({
      clip: "rect( 0px, " + this.layout.containerWidth + "px, "
                          + this.layout.containerHeight + "px, 0px)"
    });
    this.container.append(this.dashboard);
    this.container.append(this.pageindicator);

    // prevent context menus
    (this.container.get(0)).addEventListener("contextmenu", function (e) {
      e.preventDefault();
    }, true);
    ...
    this.container.get(0).addEventListener("touchstart", function(e) {
    ...
    this.container.get(0).addEventListener("touchmove", function(e) {
      if (e.touches && e.touches.length) {
        e.clientX = e.touches[0].clientX;
        e.clientY = e.touches[0].clientY;
      }
    ...
    this.container.get(0).addEventListener("touchend", function(e) {
